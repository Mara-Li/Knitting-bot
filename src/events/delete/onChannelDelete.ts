...
export default (client: Client): void => {
	client.on("channelDelete", async (channel) => {
		...
		if (channelType === ChannelType.GuildText) {
			 ...
			 */
			const allIgnore = getMaps(
				"ignore",
				TypeName.channel,
				guildID,
			) as TextChannel[];
			const allFollow = getMaps(
				"follow",
				TypeName.channel,
				guildID,
			) as TextChannel[];
			const isIgnored = allIgnore.some((ignored) => ignored.id === channel.id);
			const isFollowed = allFollow.some(
				(followed) => followed.id === channel.id,
			);
			if (isIgnored) {
				const index = allIgnore.findIndex(
			...
			if (isFollowed) {
				const index = allFollow.findIndex(
					(followed) => followed.id === channel.id,
				);
			...
			if (isCategoryIgnored) {
				const index = allCategoryIgnore.findIndex(
					(ignored) => ignored.id === channel.id,
				);
			...
			const isForumIgnored = allThreadIgnore.some(
				(ignored) => ignored.id === channel.id,
			);
			...
			if (isForumIgnored) {
				const index = allThreadIgnore.findIndex(
					(ignored) => ignored.id === channel.id,
				);
			...
			if (isForumFollowed) {
				const index = allThreadFollow.findIndex(
					(followed) => followed.id === channel.id,
				);
